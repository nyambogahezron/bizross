...
export default function HowItWorks() {
	return (
		<section id="how-it-works" className="section relative overflow-hidden">
			{/* Decorative divider */}
			<div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-px bg-gradient-to-r from-transparent via-secondary/40 to-transparent" />
			...
			<div className="max-w-7xl mx-auto">
				...
				>
					<motion.div
						variants={fadeUp}
						className="inline-flex items-center gap-2 px-3 py-1 rounded-full border badge-secondary text-xs font-medium mb-4"
					>
						✦ How It Works
					</motion.div>
					<motion.h2
					...
					>
						No training needed. If you can use a smartphone, you can use
						BizRoss.
					</motion.p>
				</motion.div>
				...
				<div className="relative">
					{/* Connector line */}
					<div className="hidden lg:block absolute top-16 left-1/2 -translate-x-1/2 h-px w-3/4 bg-gradient-to-r from-primary/30 via-secondary/30 to-success/30" />
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
						{steps.map((step, i) => (
							...
							>
								<div className="glass border border-overlay-border rounded-2xl p-8 h-full hover:border-overlay-border-hover transition-all duration-300 hover:shadow-xl">
									{/* Step number */}
									<div className="flex items-center gap-4 mb-6">
										<div
											className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${step.color} flex items-center justify-center text-foreground shadow-lg group-hover:scale-110 transition-transform duration-300`}
										>
											{step.icon}
										</div>
									...
									<div
										className={`absolute bottom-0 left-4 right-4 h-px bg-gradient-to-r ${step.color} opacity-0 group-hover:opacity-50 transition-opacity duration-500`}
									/>
								</div>
							</motion.div>
						))}
