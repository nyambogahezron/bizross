...
const perks = [
  { Icon: Globe, label: 'Remote-first', color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
  { Icon: HeartPulse, label: 'Health coverage', color: 'text-rose-400', bg: 'bg-rose-500/10' },
  { Icon: BookOpen, label: 'Learning budget', color: 'text-violet-400', bg: 'bg-violet-500/10' },
  ...
  return (
    <main className="relative bg-[#0a0c16] min-h-screen text-[#f0f2ff]">
      ...
      <section className="pt-36 pb-16 px-6 text-center">
        <p className="text-sm font-semibold tracking-widest uppercase text-indigo-400 mb-4">Join the Team</p>
      ...
      <section className="py-10 px-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {openings.map((job) => (
            <div
              key={job.title}
              className="bg-white/5 border border-white/10 rounded-2xl px-7 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 hover:border-indigo-500/40 transition-colors duration-300"
            >
              <div>
              ...
              <button className="shrink-0 bg-indigo-600 hover:bg-indigo-500 transition-colors text-white text-sm font-medium px-5 py-2.5 rounded-lg">
                Apply Now
              </button>
            </div>
          ))}
